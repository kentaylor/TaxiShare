...
ob_start();
 ...
 if (trim($msg) == "cancel" OR strtoupper(trim($msg)) == "CALL")
 {
    ...
    if($result > 0)
    {
       ...
       $url = $urlsmsbullet.urlencode($phone_number)."&m=".urlencode($msg)."&o=0061447100264";
...
//else
 ...
 if ($foundphonenumber !="")
 {
    ...
    $url = $urlsmsbullet.urlencode($phone_number)."&m=".urlencode($msgfirst)."&o=0061447100264";//  --> send to second person e.g. Jim
    echo $url;
    ...
    $url = $urlsmsbullet.urlencode($firstfoundphonenumber)."&m=".urlencode($msgsecond)."&o=0061447100264";// --> send to first person e.g. Michael
    echo $url;
