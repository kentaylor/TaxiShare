...
<script type="text/javascript">
	...
	var centerLat;
	var centerLng;
	var airportname ="Tullamarine Melbourne Airport";
	var cur_sms_format;
...
function load() {
 if (GBrowserIsCompatible()) {
		...
		map.setCenter(new GLatLng(144.95635986328125, -37.79187979395208), 12);  
                map.setUIToDefault();
                showStartingMarker();
      	}
}
			...
			function(point) {
				if (!point) {
					// do something
                                	...
                                	var centerLat = -37.79187979395208;
                                        var centerLng =144.95635986328125;

                                      if ((point.lng() >= centerLng - 0.60) && (point.lng() <= centerLng + 0.60))
                                      {
                                         if ((point.lat() >= centerLat - 0.60) && (point.lat() <= centerLat + 0.60))
                                        {
                                                 var count = 0;
...
//alert(point.lat());
                                      if ((point.lng() >= centerLng - 0.60) && (point.lng() <= centerLng + 0.60))
                                      //if ((point.lng() >= -37.99187979395208) && (point.lng() <= -37.59187979395208))
                                      {
                                        //alert("Here1"); 
					//if ((point.lat() >= 144.85635986328125) && (point.lat() <= 145.05635986328125))
                                        if ((point.lat() >= centerLat - 0.60) && (point.lat() <= centerLat + 0.60))
					{
