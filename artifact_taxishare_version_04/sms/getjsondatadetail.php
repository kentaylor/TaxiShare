...
$arr = array();
$strQuery = "SELECT upper(Q.contact_name) as contact_name, P.id, P.phone_number,upper((CASE location WHEN location THEN location ELSE sms_format END)) AS sms_format,date_time,status_code,loc_start " .
            "FROM " .
            ...
            ",sms_format, date_time,status_code,loc_start FROM incomingsmslog WHERE (status_code = 0 or status_code = 1) AND DATE_ADD(date_time, INTERVAL 15 MINUTE)>=NOW() ORDER BY sms_format, id DESC) P LEFT JOIN contactnumber Q on P.phone_number = Q.phone_number ";
